...
const SignUp = () => {
  ...
  const [loginInfo, setLoginInfo] = useState<AccountObject>({
    userName: null,
    password: null,
    walletId: null,
  })
  ...
  const showConfirmPassword = () => {
    setConfirmPasswordShown((currVal: boolean) => !currVal)
  }
  ...
  const handleSubmitChecks = async (loginInfo: AccountObject): Promise<boolean> => {
    if (loginInfo.password === confirmPassword) {
      if (Web3.utils.isAddress(loginInfo.walletId)) {
        if (true) { // check if wallet already registered
          if (true) { // check if userName already registered
            return true
          }
          else {
            return false; 
  ...
  const onSubmit = async (e: any) => {
      await handleSubmitChecks(loginInfo).then((response: any) => {
      }).then((response: any) => {
        axios.post('REGISTER_URL', {'userName': loginInfo.userName, 'password': loginInfo.password,  'walletId': loginInfo.walletId})
          .then((response: any) =>{})
  ...
  return (
    <div className='bg-page-bg w-screen h-full'>
      <div className='w-full h-min'>
        <TopNavigation/>
      </div>
      <div className='w-full h-full pt-16'>
        <div className='grid place-items-center h-full w-full pt-8 gap-y-8'>
        <div className='bg-white w-1/4 border-2 h-min rounded-lg pt-16'>
          ...
          <div className='w-full flex flex-col items-center pt-8 gap-y-4'>
            ...
            <input 
              ...
              <div className='w-2/3 grid grid-cols-10'>
                ...
                />
                <button 
                  className={`w-full bg-page-bg rounded-r-md pr-4  outline-0`} onClick={showConfirmPassword}>
                  {confirmPasswordShown ? <AiOutlineEye color="#002452" size={"16"}/> : <AiOutlineEyeInvisible color="#002452" size={"16"}/>}
                </button>
              </div>
          </div>
          <div className='w-full flex flex-col items-center pt-8'>
            <button type="button" className="w-2/3 text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2"
